// scr/core/fs.js
let savedFS = localStorage.getItem("_notroid_fs");
const defaultFS = {
    "$": { // Raíz
        "Notroid": {
            "system": {
                ...
                "notkrnl.nsh": ["LOG [OS] NOTKRNL inicializado\nEXEC Notroid/system/drivers/display.nsh", { created: new Date().toISOString(), modified: new Date().toISOString() }],
                "drivers": {
            ...
            "apps": {
                "Calculator.json": ["{}", { created: new Date().toISOString(), modified: new Date().toISOString() }]
            }
        },
        "home": {
            "user": {
...
NotroidFS._save = function(){
    localStorage.setItem("_notroid_fs", JSON.stringify(NotroidFS))
}
